...
public class Odometer extends Thread {
	...
	public void run() {
		...
		while (true) {
			...
			double tachoL = Lab2.leftMotor.getTachoCount(); //live sample
			double tachoR = Lab2.rightMotor.getTachoCount();
			double leftDistance = ( (tachoL - lastTachoL) / 180 ) * Math.PI * Lab2.WHEEL_RADIUS; //distance travelled since last iteration
			double rightDistance = ( (tachoR - lastTachoR) / 180 ) * Math.PI * Lab2.WHEEL_RADIUS;
			lastTachoL = tachoL; //reset variable
			lastTachoR = tachoR;
			double deltaD = 0.5*(leftDistance + rightDistance); //change in distance traveled
			double deltaT = (leftDistance - rightDistance) / Lab2.TRACK; //change in facing angle rad
